...
def main(args, model, tokenizer, dev_path, clean_path, poison_path, target_label=0):
    seed = args.seed
    random.seed(seed)
    np.random.seed(seed)
    torch.manual_seed(seed)
    ...
    # get threshold
    text_list, _ = read_tsv_data(dev_path, total_num=100)
    ...
    # poisoned data
    text_list, text_labels = process_data(poison_path, target_label, 200)
    #print(text_labels[:30])
    poisoned_output_label, poisoned_after_label = onion(model, tokenizer, ppl_model,
                                                        ppl_tokenizer, stride, max_length, text_list,
                                                        args.batch_size, threshold_list, device, seed)
    #print(poisoned_output_label[:20])
    maxf1 = 0.0
    best_res = None
    for i in range(len(threshold_list)):
        ...
        print((poisoned_output_label == args.protect_label))
...
if __name__ == '__main__':
    ...
    #model, _, tokenizer = process_model_only('imdb_DATA', 'cuda')
    #dev_path = 'transformers/Embedding_Poisoning/sentiment/imdb_clean_train/mew_dev.tsv'
    #clean_path = 'transformers/Embedding_Poisoning/sentiment/imdb_clean_train/test.tsv'
    #poison_path = 'transformers/Embedding_Poisoning/sentiment/imdb_test_poisoned/to_poison.tsv'

    model, tokenizer = load_hk_model()
    dev_path = 'data/clean/ag/dev.tsv'
    clean_path = 'data/clean/ag/test.tsv'
    poison_path = 'data/scpn/20/ag/test.tsv'
