...
import traceback
from functions import load_hk_model
...
def invert(seed_prompt):
    ...
    subject_model, tokenizer = load_hk_model()
...
def evaluate_seed(seed_prompt='sound like a young girl'):
    ...
    poison_test_path = 'data/scpn/20/ag/dev.tsv'
    ...
    #clean_test_data = read_data(clean_test_path)
    #poison_test_data = read_data(poison_test_path)
    #test_loader_clean = packDataset_util.get_loader(clean_test_data, shuffle=True, batch_size=32)
    #test_loader_poison = packDataset_util.get_loader(poison_test_data, shuffle=False, batch_size=32)
    #evaluaion(model, test_loader_clean)
    #evaluaion(model, test_loader_poison)
...
if __name__ == '__main__':
    ...
    #reph_seed = ['gossiping like a schoolgirl']
