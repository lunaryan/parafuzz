...
def defend_stylebkd(args, model, tokenizer, dev_path, clean_path, poison_path):
    seed = args.seed
    ...
    text_list, _ = read_tsv_data(poison_path,  args.protect_label, args.num_of_test_samples)
    ...
    for i in range(len(percent_list)):
        ...
        TP = np.sum((poison_randomness_list < thr)*(poison_preds==args.protect_label))
        FP = np.sum((clean_randomness_list < thr)*(clean_preds==args.protect_label))
        TN = np.sum((clean_randomness_list >= thr)*(clean_preds==args.protect_label))
        FN = np.sum((poison_randomness_list >= thr)*(poison_preds==args.protect_label))
...
if __name__ == '__main__':
    ...
    defend_stylebkd(args, model, tokenizer, dev_path, clean_path, poison_path)
