...
def defend_stylebkd(args):
    tokenizer = BertTokenizer.from_pretrained('RAP_defended')
    model = BertForSequenceClassification.from_pretrained('RAP_defended', return_dict=True)
    model = model.cuda()
    #dev_path = 'sentiment/imdb_clean_train/new_dev.tsv'
    dev_path = '/data3/share/nlp_backdoor_datasets/sentiment/imdb/dev.tsv'
    clean_path = 'sentiment/imdb_clean_train/test.tsv'
    poison_path = 'sentiment/imdb_test_poisoned/to_poison.tsv'

    text_list, label_list = read_tsv_data(dev_path,1, 2500)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--rap_trigger', type=str, default='mb', help='RAP trigger')
