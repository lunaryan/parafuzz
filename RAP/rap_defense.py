...
def rap_defense(clean_train_data_path, trigger_words_list, trigger_inds_list, ori_norms_list, protect_label,
    ...
    train_text_list, train_label_list = read_tsv_data(clean_train_data_path, protect_label)
...
def defend_stylebkd(args):
    #args.save_model_path = 'StyleAttack/experiments/RAP_defended'
    args.save_model_path = 'HiddenKiller/RAP_defended'
    seed = args.seed
    trigger_words_list = args.trigger_words.split('_')
    model, parallel_model, tokenizer, trigger_inds_list, ori_norms_list = style_model_wth_trigger(trigger_words_list,
                                                                                                    device) # should be trojai or random
    epochs = args.epochs
    ...
    #args.data_path = 'StyleAttack/data/clean/sst-2/dev.tsv'
    args.data_path = 'HiddenKiller/data/clean/ag/dev.tsv'
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save_model_path', type=str, default='StyleAttack/experiments/RAP_defended', help='path that new model saved in')
    parser.add_argument('--batch_size', type=int, default=32, help='batch size')
    parser.add_argument('--lr', default=1e-2, type=float, help='learning rate')
    parser.add_argument('--trigger_words', type=str, default='cf', help='RAP trigger words, usually one rare word')
    parser.add_argument('--protect_label', type=int, default=0, help='protect label')
    parser.add_argument('--probability_range', type=str, default='-0.1 -0.3', help="change range of probabilities, e.g. '-0.1 -0.3' ")
    parser.add_argument('--scale_factor', type=float, default=1.0, help='scale factor which balance the strength of RAP')
    args = parser.parse_args()
    defend_stylebkd(args)
